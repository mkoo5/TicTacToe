...
public class TTTTest {
	...
	static int miniMaxValue(char b[][], boolean player, int board, int depth) {
		...
		if (player) {
			...
			for (int i = 1; i < 10; i++) {
				if (b[board][i] == '_') {
					...
					int value = miniMaxValue(b, !player, i - 1, depth + 1);
		...
		if (isPlayerX) {
			...
			for (int i = 1; i < 10; i++) {
				if (b[board][i] == '_') {
					...
					// optimal move
					int move = miniMaxValue(b, true, i - 1, depth + 1);
					// resets move
					b[board][i] = '_';
			...
			for (int i = 1; i < 10; i++) {
				if (b[board][i] == '_') {
					...
					// stores utility value of move assuming player makes
					// optimal move
					int move = miniMaxValue(b, false, i - 1, depth + 1);
					// resets move
					b[board][i] = '_';
					if (move > bestVal) {
						answer = "" + i + move;
